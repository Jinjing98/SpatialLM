...
try:
    from spatiallm.model.spatial_attention import SpatialQwen2Attention
...
class SpatialLMQwenForCausalLM(Qwen2ForCausalLM):
    ...
    def __init__(self, config):
        ...
        if vlm_pe == "3D_RoPE":
            print("\n" + "="*80)
            print("[3D_RoPE] Replacing attention layers with SpatialQwen2Attention...")
            ...
            for layer_idx, layer in enumerate(self.model.layers):
                ...
                new_attn = SpatialQwen2Attention(config, layer_idx)
                ...
                print(f"  Layer {layer_idx}: {old_type} → SpatialQwen2Attention ✓")
            ...
            for layer_idx, layer in enumerate(self.model.layers):
                if not isinstance(layer.self_attn, SpatialQwen2Attention):
                    raise RuntimeError(
                        f"[3D_RoPE] FATAL ERROR: Layer {layer_idx} attention was NOT replaced! "
                        f"Expected SpatialQwen2Attention, got {type(layer.self_attn).__name__}. "
                        f"This means 3D_RoPE will NOT work!"
                    )
            print("[3D_RoPE] ✓ Verification passed: All attention layers are SpatialQwen2Attention\n")
        else:
            # vlm_pe is not "3D_RoPE" - use 100% original attention implementation
            print(f"[SpatialLM] vlm_pe={vlm_pe} - Using 100% original Qwen2Attention (no custom modifications)\n")
    ...
    def forward(
        ...
        **loss_kwargs,
            ...
            if self.rotary_emb_3d is not None:
                ...
                if self.rotary_emb_3d.merge_rule == "3D_with_1D" and hasattr(self, '_point_patch_coords_for_rope'):
                    ...
                    if len(self._point_token_positions) > 0:
                        assert len(self._point_token_positions) == 1, 'only support single batch for now'
        ...
        # Instead of monkey-patching, we use custom SpatialQwen2Attention layers
