...
from transformers import (
    # JJ: We will create custom versions with SOPE
    # Qwen2Model,
    # Qwen2ForCausalLM,
    AutoConfig,
...
# JJ: Import our custom SOPE versions
from spatiallm.model.sope.sope3dQwen2ForCausallm import (
    Qwen2ModelSope,
    Qwen2ForCausalLMSope,
)
...
class SopeSpatialLMQwenConfig(Qwen2Config):
    model_type = "sope_spatiallm_qwen"


class SopeSpatialLMQwenForCausalLM(Qwen2ForCausalLMSope):
    config_class = SopeSpatialLMQwenConfig

    def __init__(self, config):
        # JJ: Setup SOPE configuration BEFORE calling super().__init__
        # This is required because parent class will create Qwen2ModelSope which needs these configs
        sope_configs = getattr(config, 'sope_configs', {})
        
        def _get_config_value(key, default):
            """Helper to get config value from sope_configs or config attributes"""
            if sope_configs and key in sope_configs:
                return sope_configs[key]
            return getattr(config, key, default)
        
        # Spherical coordinate system configs (SOPE)
        config.sope_coordinate_system = _get_config_value('sope_coordinate_system', 'spherical')
        config.freq_splits = _get_config_value('freq_splits', {
            't': [0, 16], 'r': [16, 32], 'theta': [32, 48], 'phi': [48, 64]
        })
        config.spherical_norm_strategy = _get_config_value('spherical_norm_strategy', 'minmax')
        config.spherical_scale_factor = _get_config_value('spherical_scale_factor', 22.0)
        
        logger.info(f"[SOPE] Coordinate System: {config.sope_coordinate_system}")
        logger.info(f"[SOPE] freq_splits={config.freq_splits}, norm_strategy={config.spherical_norm_strategy}, scale_factor={config.spherical_scale_factor}")
        
        # JJ: Call parent class __init__ which will create Qwen2ModelSope with SOPE configs
    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        # JJ: Track point coordinates and masks for SOPE
        point_coords_list = []
        ...
        #     debug_file = "sope_debug_prefill.json"
...
AutoConfig.register("sope_spatiallm_qwen", SopeSpatialLMQwenConfig)
AutoModelForCausalLM.register(SopeSpatialLMQwenConfig, SopeSpatialLMQwenForCausalLM)
